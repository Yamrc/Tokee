	...
	return (
		<section class="card-base card-shadow p-5">
			<div class="flex items-center gap-2.5 mb-4">
				<Icon icon="material-symbols:history" width="1.5rem" height="1.5rem" class="text-[var(--primary)]" aria-hidden="true" />
				<h3 class="text-90 text-lg font-semibold">近期事件</h3>
			</div>
			<ul class="space-y-0 list-none">
				<For each={props.logs.slice(0, visibleCount())}>
					{(log) => (
						<li class="flex items-start gap-3 py-3 hover:bg-[var(--btn-plain-bg-hover)] -mx-2 px-2 rounded-md transition-colors">
							<div class="flex-1 min-w-0">
								<div class="flex items-center gap-2 mb-1.5 flex-wrap">
									<span class={`flex items-center gap-1.5 text-sm font-semibold ${logClassColors[log.class] || 'text-[var(--text-secondary)]'}`}>
										{logIcons[log.label] && <Icon icon={logIcons[log.label]} width="1rem" height="1rem" aria-hidden="true" />}
					...
					{(() => {
						...
						return (
							<button
								...
								aria-label={`展开更多 ${nextBatch} 条事件`}
							>
								<Icon icon="material-symbols:expand-more" width="1rem" height="1rem" aria-hidden="true" />
								展开更多 ({nextBatch} 条)
							</button>
						);
					})()}
